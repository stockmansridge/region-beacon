-- 02_extend_get_public_event_by_domain_nav_background_color.sql
-- DRAFT ONLY. Do not execute against production without approval.
--
-- Run AFTER 01_event_branding_nav_background_color.sql AND after the
-- existing text/card colour migrations.
--
-- Extends public.get_public_event_by_domain(_hostname) to surface the
-- new nav_background_color column so public passport pages can read it
-- via useEventBrandingKeys.
...
as $$
  ...
  select
    ...
    b.card_text_color                 as card_text_color,
    b.card_muted_text_color           as card_muted_text_color,
    b.border_color                    as border_color,
    b.primary_text_color              as primary_text_color,
    b.nav_background_color            as nav_background_color
  from resolved r
