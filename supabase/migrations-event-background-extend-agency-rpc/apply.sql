-- Extend public.get_public_event_by_agency_and_slug(_sub, _event_slug) to
-- also surface palette_key, page_background_key, page_background_color and
-- card_background_color, so the tenant slug route (/t/:agency/e/:event)
-- can render the saved palette and background the same way the subdomain
-- public route already does via get_public_event_by_domain.
--
-- Safety:
--   - SECURITY DEFINER + STABLE preserved.
--   - Projection adds 4 NEW columns at the end. Order of existing columns
--     is preserved (clients reading by name continue to work; clients
--     reading by index were already keyed off shape).
--   - RETURNS TABLE signature changes, so drop+create is required.
--   - Grants restored to anon + authenticated.
