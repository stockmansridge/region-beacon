...
--
-- Cause: RETURNS TABLE(... event_id uuid, ...) declares an OUT column
-- named `event_id` that is in scope inside the PL/pgSQL body. Several
-- statements reference `event_id` unqualified against public.passports
-- and public.participant_point_awards, which also have an `event_id`
-- column. Postgres cannot disambiguate and aborts.
--
-- Fix: fully qualify every column reference against its table, and
-- alias the passport lookup so the SELECT list is unambiguous. No
-- behaviour change; same input, same output shape.
