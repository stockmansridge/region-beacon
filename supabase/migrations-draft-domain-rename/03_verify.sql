...
-- expect:
--   getstamped.com.au      | platform_marketing | active
--   app.getstamped.com.au  | platform_admin     | active

-- 2. Marketing apex resolves.
select * from public.resolve_event_by_host('getstamped.com.au');
-- expect: kind='marketing', event_id=null, public_slug=null, requires_auth=false

-- 3. Admin host resolves (and strips :port).
select * from public.resolve_event_by_host('app.getstamped.com.au');
select * from public.resolve_event_by_host('app.getstamped.com.au:443');
...
select * from public.resolve_event_by_host('test.getstamped.com.au');
...
select * from public.resolve_event_by_host('admin.getstamped.com.au');
