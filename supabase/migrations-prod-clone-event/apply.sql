-- Clone an event (schema + venues + bonus codes + FAQ + prize rules +
-- branding + settings + announcements). Does NOT copy participants,
-- passports, visitors, checkins, bonus claims, awards history, exports.
--
...
begin
  ...
  -- Derive a unique slug for the clone (name-based, fall back to random).
  ...
  -- Build the copyable column list for `events` (all columns except the
  -- ones we override or reset).
  ...
  execute format(
    'insert into public.events (id, name, slug, public_slug, status, %s)
     select $1, $2, $3, null, ''draft'', %s
     from public.events where id = $4',
    v_cols, v_cols
  ) using v_new_event, _new_name, v_new_slug, _source_event_id;
  ...
  -- ----- venue_qr_codes (fresh tokens per venue) -----
  ...
  -- ----- event_bonus_code_venues (fresh tokens) -----
  ...
  -- ----- generic child copies (row-level, keyed by event_id, no cross refs) -----
  -- Each of these has event_id + agency_id + id + created_at/updated_at, so
  -- we copy every other column verbatim.
  for r in
    select unnest(array[
  ...
  loop
    if to_regclass('public.' || r.tbl) is not null then
      ...
      if v_cols is not null and length(v_cols) > 0 then
        -- Some child tables have unique (event_id) — that's fine because we
        -- only insert once per source row and the target event_id is new.
        execute format(
