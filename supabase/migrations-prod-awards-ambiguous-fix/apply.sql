-- Production fix: get_public_event_awards ambiguous "id" reference.
-- Symptom (public Awards page): code 42702 -- column reference "id" is ambiguous.
--
-- Cause: RETURNS TABLE declares output columns (id, event_id, ...) that
-- shadow same-named columns of tables referenced in the body. The
-- passport lookup `where id = p_passport_id` and `select event_id` are
-- ambiguous.
--
-- Fix: fully qualify every column reference. No behaviour change.
...
begin
  if p_passport_id is not null then
    select pp.event_id into v_passport_event
...
-- Verification:
-- select * from public.get_public_event_awards(
--   '41ebf116-6e70-428f-8dcb-bda56f73fb8a'::uuid,
--   null
-- );
-- -- Should return one row per active award, or zero rows, with no 42702 error.
