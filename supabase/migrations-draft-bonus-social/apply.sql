-- Bonus codes: add a "Social" kind with an @location handle and recommended
-- hashtags. The venue detail bonus block renders a "Share on socials" CTA
-- that opens the camera on mobile; the location + hashtags are surfaced so
-- customers can tag/caption their post easily.
--
...
-- Extend the public RPC to return the new fields.
