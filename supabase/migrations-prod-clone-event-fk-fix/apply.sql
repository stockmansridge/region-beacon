-- Fix: clone_event failed with
--   insert or update on table "events" violates foreign key constraint
--   "events_current_terms_fk"
--
-- The clone copies every column from the source row verbatim, which means any
-- FK on public.events that points to a per-event child (e.g. current_terms_id,
-- current_privacy_id) still references the *old* event's child row. We fix
-- this by nulling out every FK column on events (except agency_id) during the
-- initial insert. Downstream code can re-link these after use if desired.
...
begin
  ...
  -- Discover single-column FK columns on public.events, excluding agency_id.
  -- Any of these that carry over from the source would still point at the
  -- old event's child row and fail the constraint.
  ...
  where c.conrelid = 'public.events'::regclass
    ...
    and att.attname <> 'agency_id';

  -- Build SET-to-null assignments applied after the copy insert.
  if array_length(v_fk_cols, 1) is not null then
    select string_agg(quote_ident(col) || ' = null', ', ')
      into v_null_assignments
    from unnest(v_fk_cols) as col;
  end if;

  -- Copyable columns for events (excluding ones we set explicitly).
  ...
  -- Clear stale per-event FK references copied from the source row.
  if v_null_assignments <> '' then
    execute format('update public.events set %s where id = $1', v_null_assignments)
      using v_new_event;
  end if;
