-- Production fix: claim_bonus_code still fails with
--   42702 -- column reference "event_id" is ambiguous
--
-- Cause: RETURNS TABLE(... event_id uuid, ...) creates an OUT variable named
-- `event_id` inside the PL/pgSQL body. Previous fixes qualified SELECT/WHERE
-- references, but the ON CONFLICT target still contained bare `event_id`.
--
-- Fix: keep the same public signature/output shape, keep the qualified
-- extensions.digest() passport lookup, and remove the bare ON CONFLICT target.
-- The insert is idempotent via a fully-qualified NOT EXISTS check plus a
-- unique_violation fallback for concurrent scans.
